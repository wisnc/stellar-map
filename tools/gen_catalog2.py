...
def emit_segments(segs, con_of_star, fab_url):
    abbrevs = sorted({con_of_star[a] for a, b in segs} | {con_of_star[b] for a, b in segs})
    missing = [c for c in abbrevs if c not in IAU_NAMES]
    if missing:
        raise SystemExit("constellation abbrevs with no IAU name: %s" % missing)
    idx = {c: i for i, c in enumerate(abbrevs)}
    with open(os.path.join(OUT, "constellations.h"), "w") as f:
        ...
        if segs:
            ids = [idx[con_of_star[a]] for a, b in segs]
...
def main():
    ...
    emit_names(stars)
    emit_segments(segs, [x[8] for x in stars], fab_url)
